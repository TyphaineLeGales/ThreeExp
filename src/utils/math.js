 ...
 */
export const map = (n, a, b, x, y) => x + (y - x)  * (n - a) / (b - a)
export const cMap = (n, a, b, x, y) => x + (y - x)  * clamp((n - a) / (b - a), 0, 1)

export const fit = (val, min, max, toMin, toMax, ease) => {
...
export const powerTwoFloorBase = (val) => Math.floor(Math.log(val) / Math.log(2))
export const powerTwoFloor = (val) => {
  if (isPowerOfTwo(val)) return val
